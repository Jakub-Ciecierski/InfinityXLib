...
void LightGroup::Add(std::shared_ptr<LightSource> light_source) {
    ...
    switch (light_source->light_type()) {
        case LightType::SPOTLIGHT:spotlight_light_count_++;
            break;
        case LightType::DIRECTIONAL:directional_light_count_++;
            break;
        case LightType::POINT:point_light_count_++;
            break;
    }
}
...
void LightGroup::use(const Program &program) {
    ...
    for (auto &light : lights_) {
        switch (light->light_type()) {
            case LightType::SPOTLIGHT:light->use(program, spotlights++);
                break;
            case LightType::DIRECTIONAL:light->use(program, directionals++);
                break;
            case LightType::POINT:light->use(program, points++);
                break;
        }
    }
}