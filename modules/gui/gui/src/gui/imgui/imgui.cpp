...
void ImGuiIO::AddInputCharactersUTF8(const char *utf8_chars) {
    ...
    const int
        wchars_buf_len = sizeof(ImGuiIO::InputCharacters) / sizeof(ImWchar);
...
int ImGuiStorage::GetInt(ImGuiID key, int default_val) const {
    ImVector<Pair>::iterator
        it = LowerBound(const_cast<ImVector<ImGuiStorage::Pair> &>(Data), key);
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
    ...
    window->DC.PrevLineHeight = (line_height - GImGui->Style.ItemSpacing
    ...
    if (window->DC.ColumnsCount > 1)
        window->DC.ColumnsCellMinY = window->DC.CursorPos
...
const char *ImGui::GetClipboardText() {
    return GImGui->IO.GetClipboardTextFn ? GImGui->IO
        .GetClipboardTextFn(GImGui->IO.ClipboardUserData) : "";
}
...
void ImGui::NewFrame() {
    ...
    if ((g.IO.MousePos.x < 0 && g.IO.MousePos.y < 0) || (g.IO.MousePosPrev.x < 0
        && g.IO.MousePosPrev.y
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
        ...
        g.IO.MouseDownDuration[i] =
            g.IO.MouseDown[i] ? (g.IO.MouseDownDuration[i] < 0.0f ? 0.0f :
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.KeysDown); i++)
        g.IO.KeysDownDuration[i] =
            g.IO.KeysDown[i] ? (g.IO.KeysDownDuration[i] < 0.0f ? 0.0f :
    ...
    if (g.HoveredWindow
        ...
        g.HoveredRootWindow =
            g.MovedWindow ? g.MovedWindow->RootWindow : FindHoveredWindow(g.IO
                                                                              .MousePos,
                                                                          true);
    ...
    if (g.CaptureMouseNextFrame != -1)
        ...
        g.IO.WantCaptureMouse = (mouse_avail_to_imgui
            && (g.HoveredWindow != NULL || mouse_any_down)) || (g.ActiveId != 0)
            || (!g.OpenPopupStack.empty());
    g.IO.WantCaptureKeyboard =
        (g.CaptureKeyboardNextFrame != -1) ? (g.CaptureKeyboardNextFrame != 0)
                                           : (g.ActiveId != 0);
...
void ImGui::Shutdown() {
    ...
    if (g.IO
        .Fonts) // Testing for NULL to allow user to NULLify in case of running Shutdown() on multiple contexts. Bit hacky.
        g.IO.Fonts->Clear();
...
void ImGui::EndFrame() {
    ...
    IM_ASSERT(
        g.Windows.Size == g.WindowsSortBuffer.Size);  // we done something wrong
...
void ImGui::Render() {
    ...
    if (g.Style.Alpha > 0.0f) {
        ...
        if (g.IO.MouseDrawCursor) {
            const ImGuiMouseCursorData
                &cursor_data = g.MouseCursorData[g.MouseCursor];
...
void ImGui::RenderTextClipped(const ImVec2 &pos_min,
    ...
    const ImVec2 text_size =
        text_size_if_known ? *text_size_if_known : CalcTextSize(text,
                                                                text_display_end,
                                                                false,
                                                                0.0f);
    ...
    if (need_clipping) {
        ImVec4
            fine_clip_rect(clip_min->x, clip_min->y, clip_max->x, clip_max->y);
        window->DrawList->AddText(g.Font,
                                  g.FontSize,
                                  pos,
                                  GetColorU32(ImGuiCol_Text),
...
void ImGui::RenderFrame(ImVec2 p_min,
    ...
    if (border && (window->Flags & ImGuiWindowFlags_ShowBorders)) {
        window->DrawList->AddRect(p_min + ImVec2(1, 1),
                                  ...
                                  rounding);
        window->DrawList
            ->AddRect(p_min, p_max, GetColorU32(ImGuiCol_Border), rounding);
    }
}
...
void ImGui::RenderBullet(ImVec2 pos) {
    ImGuiWindow *window = GetCurrentWindow();
    window->DrawList->AddCircleFilled(pos,
                                      GImGui->FontSize * 0.20f,
                                      GetColorU32(ImGuiCol_Text),
                                      8);
}
...
bool ImGui::IsKeyReleased(int key_index) {
    ...
    if (key_index < 0) return false;
    IM_ASSERT(key_index >= 0 && key_index < IM_ARRAYSIZE(g.IO.KeysDown));
    if (g.IO.KeysDownDurationPrev[key_index] >= 0.0f
        && !g.IO.KeysDown[key_index])
        return true;
    return false;
}
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
    ...
    if (g.IO.MouseDown[button])
        if (g.IO.MouseDragMaxDistanceSqr[button]
            >= lock_threshold * lock_threshold)
            return g.IO.MousePos - g.IO
                .MouseClickedPos[button];     // Assume we can only get active with left-mouse button (at the moment).
...
void ImGui::OpenPopupEx(const char *str_id, bool reopen_existing) {
    ...
    ImGuiPopupRef popup_ref = ImGuiPopupRef(id,
                                            ...
                                            window->GetID("##menus"),
                                            g.IO
                                                .MousePos); // Tagged as new ref because constructor sets Window to NULL (we are passing the ParentWindow info here)
...
static void CloseInactivePopups() {
    ...
    if (n < g.OpenPopupStack
        .Size)   // This test is not required but it allows to set a useful breakpoint on the line below
...
bool ImGui::BeginChildFrame(ImGuiID id,
    ...
    return ImGui::BeginChild(id,
                             ...
                             (g.CurrentWindow->Flags
                                 & ImGuiWindowFlags_ShowBorders) ? true : false,
...
static ImVec2 FindBestPopupWindowPos(const ImVec2 &base_pos,
    ...
    r_outer.Reduce(ImVec2((size.x - r_outer.GetWidth() > safe_padding.x * 2)
                          ...
                          ? safe_padding.y : 0.0f));
    ImVec2
        base_pos_clamped = ImClamp(base_pos, r_outer.Min, r_outer.Max - size);
...
bool ImGui::Begin(const char *name,
    ...
    const bool
        window_appearing_after_being_hidden = (window->HiddenFrames == 1);

    // Process SetNextWindow***() calls
    bool window_pos_set_by_api = false, window_size_set_by_api = false;
    if (g.SetNextWindowPosCond) {
        const ImVec2 backup_cursor_pos = window->DC
            .CursorPos;                  // FIXME: not sure of the exact reason of this saving/restore anymore :( need to look into that.
    ...
    if (first_begin_of_the_frame) {
        ...
        window->SizeContents.x =
            (float) (int) ((window->SizeContentsExplicit.x != 0.0f) ? window
                ->SizeContentsExplicit.x : (
                               (window_is_new ? 0.0f : window->DC.CursorMaxPos.x
                                   - window->Pos.x) + window->Scroll.x));
        window->SizeContents.y =
            (float) (int) ((window->SizeContentsExplicit.y != 0.0f) ? window
                ->SizeContentsExplicit.y : (
        ...
        window->WindowPadding = ((flags & ImGuiWindowFlags_ChildWindow)
            && !(flags & (ImGuiWindowFlags_AlwaysUseWindowPadding
                ...
                | ImGuiWindowFlags_Popup))) ? ImVec2(0, 0) : style
                                    .WindowPadding;
        ...
        if (window->Collapsed) {
            ...
            if (window->AutoFitFramesX > 0)
                window->SizeFull.x =
                    window->AutoFitOnlyGrows ? ImMax(window->SizeFull.x,
                                                     size_auto_fit.x)
                                             : size_auto_fit.x;
            if (window->AutoFitFramesY > 0)
                window->SizeFull.y =
                    window->AutoFitOnlyGrows ? ImMax(window->SizeFull.y,
                                                     size_auto_fit.y)
                                             : size_auto_fit.y;
        } else {
            if ((flags & ImGuiWindowFlags_AlwaysAutoResize)
        ...
        window->Size =
            window->Collapsed ? window->TitleBarRect().GetSize() : window
                ->SizeFull;
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow)
            ...
            window->Size = window->SizeFull =
                size_on_first_use; // NB: argument name 'size_on_first_use' misleading here, it's really just 'size' as provided by user passed via BeginChild()->Begin().
        }
        ...
        if (window_pos_center) {
            ...
            if (parent_window->DC.MenuBarAppending)
                ...
                rect_to_avoid =
                    ImRect(parent_window->Pos.x + style.ItemSpacing.x,
                           -FLT_MAX,
        ...
        const float window_rounding =
            (flags & ImGuiWindowFlags_ChildWindow) ? style.ChildWindowRounding
        ...
        if (window->Collapsed) {
            ...
            if (!(flags & ImGuiWindowFlags_AlwaysAutoResize)
                ...
                resize_col =
                    GetColorU32(held ? ImGuiCol_ResizeGripActive : hovered
            ...
            if (!(flags & ImGuiWindowFlags_NoResize)) {
                ...
                window->DrawList->PathArcToFast(ImVec2(
                    br.x - window_rounding - window->BorderSize,
            ...
            if (flags & ImGuiWindowFlags_ShowBorders) {
                ...
                if (!(flags & ImGuiWindowFlags_NoTitleBar))
                    window->DrawList
                        ->AddLine(title_bar_rect.GetBL() + ImVec2(1, 0),
        ...
        window->ContentsRegionRect.Max.x =
            -window->Scroll.x - window->WindowPadding.x
                + (window->SizeContentsExplicit.x != 0.0f ? window
                    ->SizeContentsExplicit.x : (window->Size.x
                    - window->ScrollbarSizes.x));
        window->ContentsRegionRect.Max.y =
            -window->Scroll.y - window->WindowPadding.y
                + (window->SizeContentsExplicit.y != 0.0f ? window
                    ->SizeContentsExplicit.y : (window->Size.y
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
            if (p_open != NULL) {
                ...
                const float
                    rad = (window->TitleBarHeight() - pad * 2.0f) * 0.5f;
            ...
            clip_rect.Max = ImVec2(window->Pos.x + window->Size.x
                                       - (p_open ? title_bar_rect.GetHeight()
                                           - 3 : style.FramePadding.x),
                                   text_max
                                       .y); // Match the size of CloseWindowButton()
            float pad_left = (flags & ImGuiWindowFlags_NoCollapse) == 0 ? (
                style.FramePadding.x + g.FontSize + style.ItemInnerSpacing.x)
                                                                        : 0.0f;
            float pad_right =
                (p_open != NULL) ? (style.FramePadding.x + g.FontSize
                    + style.ItemInnerSpacing.x) : 0.0f;
            if (style.WindowTitleAlign.x > 0.0f)
                pad_right = ImLerp(pad_right,
                                   pad_left,
    ...
    ImRect
        clip_rect; // Force round to ensure that e.g. (int)(max.x-min.x) in user's render code produce correct result.
    ...
    clip_rect.Min.y = ImFloor(
        0.5f + title_bar_rect.Max.y + window->MenuBarHeight() + border_size);
    ...
    window->SkipItems =
        (window->Collapsed || !window->Active) && window->AutoFitFramesX <= 0
...
void ImGui::End() {
    ...
    SetCurrentWindow(g.CurrentWindowStack.empty() ? NULL : g.CurrentWindowStack
        .back());
}
...
static void Scrollbar(ImGuiWindow *window, bool horizontal) {
    ...
    if (!horizontal)
        bb.Min.y += window->TitleBarHeight()
            + ((window->Flags & ImGuiWindowFlags_MenuBar) ? window
                ->MenuBarHeight() : 0.0f);

    float window_rounding =
        (window->Flags & ImGuiWindowFlags_ChildWindow) ? style
            .ChildWindowRounding : style.WindowRounding;
    ...
    window->DrawList->AddRectFilled(bb.Min,
                                    ...
                                    window_rounding,
                                    window_rounding_corners);
    bb.Reduce(ImVec2(ImClamp((float) (int) ((bb.Max.x - bb.Min.x - 2.0f)
                         * 0.5f), 0.0f, 3.0f),
                     ImClamp((float) (int) ((bb.Max.y - bb.Min.y - 2.0f)
                         * 0.5f), 0.0f, 3.0f)));
    ...
    if (held && grab_h_norm < 1.0f) {
        ...
        float *click_delta_to_grab_center_v =
            horizontal ? &g.ScrollbarClickDeltaToGrabCenter.x : &g
                .ScrollbarClickDeltaToGrabCenter.y;
    ...
    const ImU32 grab_col =
        ImGui::GetColorU32(held ? ImGuiCol_ScrollbarGrabActive : hovered
                                                                 ? ImGuiCol_ScrollbarGrabHovered
                                                                 : ImGuiCol_ScrollbarGrab);
    if (horizontal)
        window->DrawList
            ->AddRectFilled(ImVec2(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm),
                                   bb.Min.y),
                            ImVec2(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm)
                                       + grab_h_pixels, bb.Max.y),
                            grab_col,
                            style.ScrollbarRounding);
    else
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x,
                                               ImLerp(bb.Min.y,
                                                      bb.Max.y,
                                                      grab_v_norm)),
                                        ImVec2(bb.Max.x,
...
void ImGui::PopItemWidth() {
    ...
    window->DC.ItemWidth =
        window->DC.ItemWidthStack.empty() ? window->ItemWidthDefault : window
            ->DC.ItemWidthStack.back();
}
...
void ImGui::PopFont() {
    ...
    SetCurrentFont(g.FontStack.empty() ? g.IO.Fonts->Fonts[0] : g.FontStack
        .back());
}
...
void ImGui::PopAllowKeyboardFocus() {
    ...
    window->DC.AllowKeyboardFocus =
        window->DC.AllowKeyboardFocusStack.empty() ? true : window->DC
            .AllowKeyboardFocusStack.back();
}
...
void ImGui::PopButtonRepeat() {
    ...
    window->DC.ButtonRepeat =
        window->DC.ButtonRepeatStack.empty() ? false : window->DC
            .ButtonRepeatStack.back();
}
...
void ImGui::PopTextWrapPos() {
    ...
    window->DC.TextWrapPos =
        window->DC.TextWrapPosStack.empty() ? -1.0f : window->DC
            .TextWrapPosStack.back();
}
...
static const ImGuiStyleVarInfo GStyleVarInfo[ImGuiStyleVar_Count_] =
    {
        ...
        {ImGuiDataType_Float,
         (ImU32) IM_OFFSETOF(ImGuiStyle, ChildWindowRounding)},
...
void ImGui::SetNextWindowContentWidth(float width) {
    ...
    g.SetNextWindowContentSizeVal = ImVec2(width,
                                           g.SetNextWindowContentSizeCond ? g
                                               .SetNextWindowContentSizeVal.y
                                                                          : 0.0f);
    g.SetNextWindowContentSizeCond = ImGuiSetCond_Always;
}
...
void ImGui::SetScrollY(float scroll_y) {
    ImGuiWindow *window = GetCurrentWindow();
    window->ScrollTarget.y = scroll_y + window->TitleBarHeight() + window
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio) {
    ...
    if (center_y_ratio <= 0.0f && window->ScrollTarget.y
        <= window->WindowPadding
            .y)    // Minor hack to make "scroll to top" take account of WindowPadding, else it would scroll to (WindowPadding.y - ItemSpacing.y)
        ...
        if (text_pos.y <= clip_rect.Max.y) {
            ...
            if (line < text_end) {
                ...
                while (line < text_end) {
                    ...
                    const ImVec2
                        line_size = CalcTextSize(line, line_end, false);
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args) {
    ...
    const ImRect value_bb(window->DC.CursorPos,
                          window->DC.CursorPos + ImVec2(w,
                                                        label_size.y
                                                            + style.FramePadding
                                                                .y * 2));
    const ImRect total_bb(window->DC.CursorPos,
                          window->DC.CursorPos + ImVec2(w + (label_size.x > 0.0f
                                                             ? style
                                                                 .ItemInnerSpacing
                                                                 .x : 0.0f),
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers)
            ...
            if ((flags & ImGuiButtonFlags_PressedOnRelease)
                ...
                if (!((flags & ImGuiButtonFlags_Repeat)
                    && g.IO.MouseDownDurationPrev[0] >= g.IO
                        .KeyRepeatDelay))  // Repeat mode trumps <on release>
                    pressed = true;
                SetActiveID(0);
            }
...
bool ImGui::ButtonEx(const char *label,
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y
        < window->DC
            .CurrentLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
    ...
    // Render
    const ImU32 col =
        GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos, float radius) {
    ...
    // Render
    const ImU32 col =
        GetColorU32((held && hovered) ? ImGuiCol_CloseButtonActive : hovered
...
void ImGui::Image(ImTextureID user_texture_id,
    ...
    if (border_col.w > 0.0f) {
        window->DrawList
            ->AddRect(bb.Min, bb.Max, GetColorU32(border_col), 0.0f);
...
bool ImGui::ImageButton(ImTextureID user_texture_id,
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float) frame_padding,
                                                ...
                                                : style.FramePadding;
    const ImRect
        bb(window->DC.CursorPos, window->DC.CursorPos + size + padding * 2);
    ...
    if (bg_col.w > 0.0f)
        window->DrawList
            ->AddRectFilled(image_bb.Min, image_bb.Max, GetColorU32(bg_col));
    window->DrawList->AddImage(user_texture_id,
                               image_bb.Min,
                               image_bb.Max,
                               uv0,
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags) {
    ...
    if (g.SetNextTreeNodeOpenCond != 0) {
        ...
        is_open = storage
            ->GetInt(id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) != 0;
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id,
    ...
    const float text_base_offset_y = ImMax(0.0f,
                                           window->DC.CurrentLineTextBaseOffset
                                               - padding
                                                   .y); // Latch before ItemSize changes it
    ...
    const ImRect interact_bb = display_frame ? bb : ImRect(bb.Min.x,
                                                           bb.Min.y,
                                                           bb.Min.x + text_width
                                                               + style
                                                                   .ItemSpacing
    ...
    // Render
    const ImU32 col =
        GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered
...
bool ImGui::TreeNodeExV(const char *str_id,
    ...
    const char *label_end = g.TempBuffer
        + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    return TreeNodeBehavior(window->GetID(str_id),
                            flags,
                            g.TempBuffer,
                            label_end);
}
...
bool ImGui::TreeNodeExV(const void *ptr_id,
    ...
    const char *label_end = g.TempBuffer
        + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    return TreeNodeBehavior(window->GetID(ptr_id),
                            flags,
                            g.TempBuffer,
                            label_end);
}
...
void ImGui::BulletTextV(const char *fmt, va_list args) {
    ...
    const float line_height = ImMax(ImMin(window->DC.CurrentLineHeight,
                                    ...
                                    g.FontSize);
    const ImRect bb(window->DC.CursorPos,
                    window->DC.CursorPos + ImVec2(g.FontSize
                                                      + (label_size.x > 0.0f ? (
                                                          label_size.x + style
                                                              .FramePadding.x
                                                              * 2) : 0.0f),
                                                  ImMax(line_height,
                                                        label_size
                                                            .y)));  // Empty text doesn't add padding
    ItemSize(bb);
    if (!ItemAdd(bb, NULL))
        return;
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect &aabb,
    ...
    if (text_value_changed)
        return DataTypeApplyOpFromText(buf,
                                       GImGui->InputTextState.InitialText
                                           .begin(),
                                       data_type,
                                       data_ptr,
                                       NULL);
...
bool ImGui::SliderBehavior(const ImRect &frame_bb,
    ...
    const float slider_sz =
        is_horizontal ? (frame_bb.GetWidth() - grab_padding * 2.0f) : (
            frame_bb.GetHeight() - grab_padding * 2.0f);
    float grab_sz;
    if (decimal_precision > 0)
        grab_sz = ImMin(style.GrabMinSize, slider_sz);
    else
        grab_sz = ImMin(ImMax(1.0f * (slider_sz / (v_max - v_min + 1.0f)),
                              style.GrabMinSize),
    ...
    if (v_min * v_max < 0.0f) {
        // Different sign
        const float
            linear_dist_min_to_0 = powf(fabsf(0.0f - v_min), 1.0f / power);
        const float
            linear_dist_max_to_0 = powf(fabsf(v_max - 0.0f), 1.0f / power);
            ...
            if (is_non_linear) {
                ...
                if (normalized_pos < linear_zero_pos) {
                    ...
                    if (fabsf(linear_zero_pos - 1.0f) > 1.e-6f)
                        a = (normalized_pos - linear_zero_pos)
...
bool ImGui::SliderFloat(const char *label,
    ...
    const ImRect total_bb(frame_bb.Min,
                          frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x
                                                    + label_size.x : 0.0f,
                                                0.0f));
    ...
    const bool
        tab_focus_requested = FocusableItemRegister(window, g.ActiveId == id);
    ...
    if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
        return InputScalarAsWidgetReplacement(frame_bb,
                                              label,
                                              ImGuiDataType_Float,
                                              v,
                                              id,
...
bool ImGui::VSliderFloat(const char *label,
    ...
    char value_buf[64];
    char *value_buf_end = value_buf + ImFormatString(value_buf,
...
bool ImGui::VSliderInt(const char *label,
    ...
    if (!display_format)
        display_format = "%.0f";
    float v_f = (float) *v;
    bool value_changed = VSliderFloat(label,
                                      size,
                                      &v_f,
                                      (float) v_min,
                                      (float) v_max,
...
bool ImGui::DragFloat(const char *label,
    ...
    ItemSize(total_bb, style.FramePadding.y);
    const bool value_changed = DragBehavior(frame_bb,
                                            id,
                                            v,
                                            v_speed,
                                            v_min,
...
bool ImGui::DragInt(const char *label,
    ...
    if (!display_format)
        display_format = "%.0f";
    float v_f = (float) *v;
    bool value_changed = DragFloat(label,
                                   &v_f,
                                   v_speed,
                                   (float) v_min,
                                   (float) v_max,
...
void ImGui::PlotEx(ImGuiPlotType plot_type,
    ...
    if (values_count > 0) {
        ...
        ImVec2 tp0 = ImVec2(t0,
                            1.0f - ImSaturate((v0 - scale_min) / (scale_max
...
void ImGui::PlotEx_xy(ImGuiPlotType plot_type,
                      const char *label,
                      float
                      (*values_getter)(void *data, int idx),
                      void *data,
                      void *data2,
                      int
                      values_count,
...
void ImGui::PlotLinesXY(const char *label,
                        const float *values,
                        const float *values2,
                        int
                        values_count,
...
void ImGui::ProgressBar(float fraction,
    ...
    const ImVec2
        fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
    RenderFrame(bb.Min,
                fill_br,
...
bool ImGui::RadioButton(const char *label, bool active) {
    ...
    if (window->Flags & ImGuiWindowFlags_ShowBorders) {
        window->DrawList->AddCircle(center + ImVec2(1, 1),
                                    ...
                                    16);
        window->DrawList
            ->AddCircle(center, radius, GetColorU32(ImGuiCol_Border), 16);
    }
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar *text_begin,
    ...
    while (s < text_end) {
        ...
        const float
            char_width = font->GetCharAdvance((unsigned short) c) * scale;
...
bool ImGui::InputTextEx(const char *label,
    ...
    if (focus_requested || user_clicked || user_scrolled) {
        if (g.ActiveId != id) {
            ...
            ImStrncpy(edit_state.InitialText.Data,
                      buf,
    ...
    if (g.ActiveId == id) {
        ...
        const float mouse_y =
            (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y
                - style.FramePadding.y) : (g.FontSize * 0.5f));
        ...
        const bool is_shortcut_key_only =
            (io.OSXBehaviors ? (io.KeySuper && !io.KeyCtrl) : (io.KeyCtrl
                && !io.KeySuper)) && !io.KeyAlt && !io
                .KeyShift; // OS X style: Shortcuts using Cmd/Super instead of Ctrl
        const bool is_wordmove_key_down = io.OSXBehaviors ? io.KeyAlt : io
            .KeyCtrl;                     // OS X style: Text editing cursor movement using Alt instead of Ctrl
        const bool is_startend_key_down =
            io.OSXBehaviors && io.KeySuper && !io.KeyCtrl && !io
        ...
        if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
            ...
            edit_state
                .OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask :
                              STB_TEXTEDIT_K_LINESTART | k_mask);
        }
        else if (IsKeyPressedMap(ImGuiKey_End)) {
            edit_state
                .OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTEND | k_mask :
            ...
            if (io.SetClipboardTextFn) {
                const int ib =
                    edit_state.HasSelection() ? ImMin(edit_state.StbState
                                                          .select_start,
                                                      edit_state.StbState
                                                          .select_end) : 0;
                const int ie =
                    edit_state.HasSelection() ? ImMax(edit_state.StbState
                                                          .select_start,
                                                      edit_state.StbState
                                                          .select_end)
                                              : edit_state.CurLenW;
            ...
            if (const char *clipboard = GetClipboardText()) {
                ...
                for (const char *s = clipboard; *s;) {
                    ...
                    if (c == 0)
                        break;
                    if (c >= 0x10000 || !InputTextFilterCharacter(&c,
                                                                  flags,
                                                                  callback,
                                                                  user_data))
                        continue;
        ...
        if (cancel_edit) {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion
                ...
                if (event_flag) {
                    ...
                    IM_ASSERT(callback_data.Buf == edit_state.TempTextBuffer
    ...
    const char *buf_display =
        (g.ActiveId == id && is_editable) ? edit_state.TempTextBuffer.Data
                                          : buf;
    buf = NULL;
    ...
    ImVec2 render_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min
        + style.FramePadding;
    ...
    if (g.ActiveId == id || is_currently_scrolling) {
...

        {
            ...
            cursor_offset.x = InputTextCalcTextSizeW(ImStrbolW(
                searches_input_ptr[0],
                text_begin), searches_input_ptr[0]).x;
            cursor_offset.y = searches_result_line_number[0] * g.FontSize;
            if (searches_result_line_number[1] >= 0) {
                select_start_offset.x = InputTextCalcTextSizeW(ImStrbolW(
                    searches_input_ptr[1],
                    text_begin), searches_input_ptr[1]).x;
...
bool ImGui::InputScalarEx(const char *label,
    ...
    PushID(label);
    const ImVec2
        button_sz = ImVec2(g.FontSize, g.FontSize) + style.FramePadding * 2.0f;
    if (step_ptr)
...
bool ImGui::Combo(const char *label,
    ...
    const ImRect
        value_bb(frame_bb.Min, frame_bb.Max - ImVec2(arrow_size, 0.0f));
    RenderFrame(frame_bb.Min,
                frame_bb.Max,
                GetColorU32(ImGuiCol_FrameBg),
                true,
    ...
    if (IsPopupOpen(id)) {
        ...
        float popup_height = (label_size.y + style.ItemSpacing.y)
            * ImMin(items_count, height_in_items) + (style.FramePadding.y * 3);
        float popup_y1 = frame_bb.Max.y;
        float popup_y2 = ImClamp(popup_y1 + popup_height,
                                 popup_y1,
                                 g.IO.DisplaySize.y
                                     - style.DisplaySafeAreaPadding.y);
        if ((popup_y2 - popup_y1) < ImMin(popup_height,
                                          frame_bb.Min.y
                                              - style.DisplaySafeAreaPadding
                                                  .y)) {
...
bool ImGui::Selectable(const char *label,
    ...
    ImVec2 size_draw
        ((size_arg.x != 0 && !(flags & ImGuiSelectableFlags_DrawFillAvailWidth))
         ? size_arg.x : w_draw, size_arg.y != 0.0f ? size_arg.y : size.y);
...
bool ImGui::ListBoxHeader(const char *label, const ImVec2 &size_arg) {
    ...
    ImRect bb(frame_bb.Min,
              frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x
                                                        : 0.0f,
                                    0.0f));
    window->DC.LastItemRect = bb;
...
bool ImGui::MenuItem(const char *label,
    ...
    if (selected)
        RenderCheckMark(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f,
                                     0.0f),
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        pressed = Selectable(label,
                             ...
                             ImGuiSelectableFlags_Menu
                                 ...
                                 | (!enabled ? ImGuiSelectableFlags_Disabled
                                             : 0),
                             ImVec2(w, 0.0f));
        PopStyleVar();
        SameLine();
        ...
        if (!enabled)
            PushStyleColor(ImGuiCol_Text,
                           g.Style.Colors[ImGuiCol_TextDisabled]);
        RenderCollapseTriangle(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f,
                                            0.0f),
                               false);
    ...
    if (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) {
        ...
        if (g.HoveredWindow == window
            ...
            if (ImGuiWindow *next_window =
                ...
                ImVec2 tb =
                    (window->Pos.x < next_window->Pos.x) ? next_window_rect
                        .GetTL() : next_window_rect.GetTR();
                ImVec2 tc =
                    (window->Pos.x < next_window->Pos.x) ? next_window_rect
                        .GetBL() : next_window_rect.GetBR();
    ...
    if (menu_is_open) {
        ...
        ImGuiWindowFlags flags = ImGuiWindowFlags_ShowBorders | ((window->Flags
...
bool ImGui::ColorEdit4(const char *label, float col[4], bool alpha) {
    ...
    switch (edit_mode) {
        ...
        case ImGuiColorEditMode_HSV: {
            ...
            const float
                w_items_all = w_full - (square_sz + style.ItemInnerSpacing.x);
            const float w_item_one = ImMax(1.0f,
    ...
    if (window->DC.ColorEditMode == ImGuiColorEditMode_UserSelectShowButton) {
        ...
        if (ButtonEx(button_titles[edit_mode],
                     ImVec2(0, 0),
                     ImGuiButtonFlags_DontClosePopups))
            g.ColorEditModeStorage.SetInt(id,
                                          (edit_mode + 1)
    ...
    if (label != label_display_end) {
        SameLine(0,
                 (window->DC.ColorEditMode
                     == ImGuiColorEditMode_UserSelectShowButton) ? -1.0f : style
                     .ItemInnerSpacing.x);
...
bool ImGui::IsRectVisible(const ImVec2 &size) {
    ImGuiWindow *window = GetCurrentWindowRead();
    return window->ClipRect
        .Overlaps(ImRect(window->DC.CursorPos, window->DC.CursorPos + size));
}
...
void ImGui::EndGroup() {
    ...
    group_bb.Max.y -= g.Style.ItemSpacing
...
void ImGui::NextColumn() {
    ...
    if (++window->DC.ColumnsCurrent < window->DC.ColumnsCount) {
        ...
        window->DC.ColumnsOffsetX =
            GetColumnOffset(window->DC.ColumnsCurrent) - window->DC.IndentX
...
float ImGui::GetColumnOffset(int column_index) {
    ...
    if (g.ActiveId) {
        const ImGuiID
            column_id = window->DC.ColumnsSetId + ImGuiID(column_index);
...
static void PushColumnClipRect(int column_index) {
    ...
    float x2 = ImFloor(
        0.5f + window->Pos.x + ImGui::GetColumnOffset(column_index + 1) - 1.0f);
...
void ImGui::Columns(int columns_count, const char *id, bool border) {
    ...
    if (window->DC.ColumnsCount != columns_count && window->DC.ColumnsCount != 1
        ...
        for (int i = 1; i < window->DC.ColumnsCount; i++) {
            ...
            window->DrawList
                ->AddLine(ImVec2(xi, y1 + 1.0f), ImVec2(xi, y2), col);
    ...
    const float content_region_width =
        (window->SizeContentsExplicit.x != 0.0f) ? window->SizeContentsExplicit
            .x : window->Size.x;
    ...
    window->DC.ColumnsMaxX = content_region_width - window->Scroll.x
        - ((window->Flags & ImGuiWindowFlags_NoScrollbar) ? 0 : g.Style
            .ScrollbarSize);// - window->WindowPadding().x;
    window->DC.ColumnsStartPosY = window->DC.CursorPos.y;
    window->DC.ColumnsCellMinY = window->DC.ColumnsCellMaxY =
        window->DC.CursorPos.y;
    window->DC.ColumnsOffsetX = 0.0f;
    ...
    if (window->DC.ColumnsCount != 1) {
        ...
        for (int column_index = 0; column_index < columns_count + 1;
            ...
            const float
                default_t = column_index / (float) window->DC.ColumnsCount;
...
static void SetClipboardTextFn_DefaultImpl(void *, const char *text) {
    ...
    g.PrivateClipboard =
        (char *) ImGui::MemAlloc((size_t) (text_end - text) + 1);
    ...
    if (ImGui::Begin("ImGui Metrics", p_open)) {
        ...
        struct Funcs {
            static void NodeDrawList(ImDrawList *draw_list, const char *label) {
                ...
                ImDrawList *overlay_draw_list = &GImGui
                ...
                for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
                    ...
                    ImDrawIdx *idx_buffer =
                        (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer
                            .Data : NULL;
                    bool pcmd_node_open = ImGui::TreeNode((void *) (pcmd
        ...
        if (ImGui::TreeNode("Basic state")) {
            ...
            ImGui::Text("ActiveID: 0x%08X/0x%08X",
                        g.ActiveId,
                        g.ActiveIdPreviousFrame);
            ImGui::TreePop();
        }
