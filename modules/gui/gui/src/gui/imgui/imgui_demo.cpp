...
void ImGui::ShowTestWindow(bool *p_open) {
    ...
    if (ImGui::CollapsingHeader("Widgets")) {
        ...
        if (ImGui::TreeNode("Word Wrapping")) {
            ...
            ImGui::GetWindowDrawList()
                ->AddRectFilled(ImVec2(pos.x + wrap_width, pos.y),
...

        {
            ...
            ImGui::SliderFloat("slider log float",
                               &f2,
                               -10.0f,
                               10.0f,
                               "%.4f",
        ...
        if (ImGui::TreeNode("Vertical Sliders")) {
            ...
            static float
                values[7] = {0.0f, 0.60f, 0.35f, 0.9f, 0.70f, 0.20f, 0.0f};
            ImGui::PushID("set1");
            for (int i = 0; i < 7; i++) {
                if (i > 0) ImGui::SameLine();
                ImGui::PushID(i);
            ...
            const int rows = 3;
            const ImVec2
                small_slider_size(18, (160.0f - (rows - 1) * spacing) / rows);
    ...
    if (ImGui::CollapsingHeader("Layout")) {
        ...
        if (ImGui::TreeNode("Horizontal Scrolling")) {
            ...
            ImGui::BeginChild("scrolling",
                              ImVec2(0,
                                     ImGui::GetItemsLineHeightWithSpacing() * 7
                                         + 30),
                              true,
                              ImGuiWindowFlags_HorizontalScrollbar);
            for (int line = 0; line < lines; line++) {
                ...
                for (int n = 0; n < num_buttons; n++) {
                    ...
                    char num_buf[16];
                    const char *label =
                        (!(n % 15)) ? "FizzBuzz" : (!(n % 3)) ? "Fizz" : (!(n
                            % 5)) ? "Buzz" : (sprintf(num_buf,
                                                      "%d",
                                                      n), num_buf);
                    float hue = n * 0.05f;
                    ImGui::PushStyleColor(ImGuiCol_Button,
    ...
    if (ImGui::CollapsingHeader("Filtering")) {
        ...
        filter.Draw();
        const char *lines[] =
            {"aaa1.c", "bbb1.c", "ccc1.c", "aaa2.cpp", "bbb2.cpp", "ccc2.cpp",
             "abc.h", "hello, world"};
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus")) {
        ...
        if (ImGui::TreeNode("Dragging")) {
            ...
            if (ImGui::IsItemActive()) {
                ...
                draw_list
                    ->AddLine(ImGui::CalcItemRectClosestPoint(ImGui::GetIO()
                                                                  .MousePos,
                                                              true,
                                                              -2.0f),
                              ImGui::GetIO().MousePos,
                              ImColor(ImGui::GetStyle()
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref) {
    ...
    if (ImGui::TreeNode("Settings")) {
        ...
        ImGui::SliderFloat("WindowRounding",
                           ...
                           16.0f,
                           "%.0f");
        ImGui::SliderFloat("ChildWindowRounding",
    ...
    if (ImGui::TreeNode("Colors")) {
        ...
        if (ImGui::Button("Copy Colors")) {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++) {
                ...
                if (!output_only_modified || memcmp(&col,
                                                    (ref ? &ref->Colors[i]
                                                         : &default_style
                                                            .Colors[i]),
                                                    sizeof(ImVec4)) != 0)
        ...
        for (int i = 0; i < ImGuiCol_COUNT; i++) {
            ...
            if (memcmp(&style.Colors[i],
                ...
                if (ImGui::Button("Revert"))
                    style.Colors[i] = ref ? ref->Colors[i] : default_style
                        .Colors[i];
                if (ref) {
                    ImGui::SameLine();
                ...
                if (ImGui::TreeNode("Glyphs",
                    ...
                    for (int base = 0; base < 0x10000; base += 256) {
                        ...
                        if (count > 0
                            ...
                            ImVec2 cell_size
                                (font->FontSize * 1, font->FontSize * 1);
                            ImVec2 base_pos = ImGui::GetCursorScreenPos();
                            ImDrawList *draw_list = ImGui::GetWindowDrawList();
                            for (int n = 0; n < 256; n++) {
                                ImVec2 cell_p1(base_pos.x + (n % 16)
                                                   * (cell_size.x + cell_spacing),
                                               base_pos.y + (n / 16)
                                                   * (cell_size.y
                                                       + cell_spacing));
...
static void ShowExampleAppCustomRendering(bool *p_open) {
    ...
    ImGui::ColorEdit3("Color", &col.x);
    {
        ...
        for (int n = 0; n < 2; n++) {
            ...
            x += sz + spacing;
            draw_list
                ->AddLine(ImVec2(x, y), ImVec2(x + sz, y), col32, thickness);
            x += sz + spacing;
            ...
            x += sz + spacing;
            draw_list
                ->AddLine(ImVec2(x, y), ImVec2(x, y + sz), col32, thickness);
            x += spacing;
        ...
        x += sz + spacing;
        draw_list
            ->AddRectFilled(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f);
        x += sz + spacing;
    ...
    ImGui::Separator();
    {
        ...
        draw_list->PushClipRect(canvas_pos,
                                ImVec2(canvas_pos.x + canvas_size.x,
                                       canvas_pos.y + canvas_size
                                           .y));      // clip lines within the canvas (if we resize it, etc.)
...
struct ExampleAppConsole {
    ...
    ExampleAppConsole() {
        ...
        Commands
            .push_back("CLASSIFY");  // "classify" is here to provide an example of "C"+[tab] completing to "CL" and displaying matches.
    ...
    void Draw(const char *title, bool *p_open = NULL) {
        ...
        if (Filter.IsActive()) {
            ...
            for (int line_no = 0; line != NULL; line_no++) {
                const char *line_end = (line_no < LineOffsets.Size) ? buf_begin
                    + LineOffsets[line_no] : NULL;
