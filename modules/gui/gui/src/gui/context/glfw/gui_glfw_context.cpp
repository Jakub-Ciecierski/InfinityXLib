...
bool GUIGLFWContext::Init(void *native_window,
    ...
    auto glfw_control_context = std::static_pointer_cast<ControlContextGLFW>(control_context);
    if(!glfw_control_context)
        throw std::invalid_argument("ControlContextGLFW required");

    glfw_control_context->SetExternalCallbacks(glfw_callbacks::ExternalCallbacks{
