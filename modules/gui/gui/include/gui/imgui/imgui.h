...
typedef int
    ImGuiCol;               // a color identifier for styling       // enum ImGuiCol_
typedef int
    ImGuiStyleVar;          // a variable identifier for styling    // enum ImGuiStyleVar_
typedef int
    ImGuiKey;               // a key identifier (ImGui-side enum)   // enum ImGuiKey_
typedef int
...
typedef void
(*ImGuiSizeConstraintCallback)(ImGuiSizeConstraintCallbackData *data);
...
IMGUI_API void PlotLinesXY(const char *label,
                           const float *values,
                           const float *values2,
                           int values_count,
                           int values_offset = 0,
...
IMGUI_API bool TreeNode(const void *ptr_id, const char *fmt, ...) IM_PRINTFARGS(
    2);    // "
...
struct ImGuiIO {
    ...
    ];   // <unset>              // Map of indices into the KeysDown[512] entries array
    float
    ...
    float
        MouseWheel;                 // Mouse wheel: 1 unit scrolls about 5 lines text.
    bool
    ...
    bool
        KeySuper;                   // Keyboard modifier pressed: Cmd/Super/Windows
    bool KeysDown[512
    ...
    bool
        WantCaptureMouse;           // Mouse is hovering a window or widget is active (= ImGui will use your mouse input)
    bool
        WantCaptureKeyboard;        // Widget is active (= ImGui will use your keyboard input)
    bool
        WantTextInput;              // Some text input widget is active, which will read input characters from the InputCharacters array.
    float
        Framerate;                  // Framerate estimation, in frame per second. Rolling average estimation based on IO.DeltaTime over 120 frames
    ...
    int
        MetricsActiveWindows;       // Number of visible windows (exclude child windows)
    ...
    ImVec2
        MouseDelta;                 // Mouse delta. Note that this is zero if either current or previous position are negative to allow mouse enabling/disabling.
...
struct ImGuiTextEditCallbackData {
    ...
    int
        SelectionStart; //                                      // Read-write (== to SelectionEnd when no selection)
...
struct ImDrawList {
    ...
    inline void PathFill(ImU32 col) {
        AddConvexPolyFilled(_Path.Data,
                            _Path.Size,
                            col,
                            true);
        PathClear();
...
struct ImDrawData {
    ...
    int CmdListsCount;
    int
        TotalVtxCount;          // For convenience, sum of all cmd_lists vtx_buffer.Size
    int
        TotalIdxCount;          // For convenience, sum of all cmd_lists idx_buffer.Size
...
struct ImFontConfig {
    ...
    float
        SizePixels;                 //          // Size in pixels for rasterizer
    int OversampleH,
    ...
    bool
        MergeGlyphCenterV;          // false    // When merging (multiple ImFontInput for one ImFont), vertically center new glyphs instead of aligning their baseline

    // [Internal]
    char
        Name[32];                               // Name (strictly for debugging)
    ImFont *DstFont;
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFont *AddFontFromMemoryCompressedTTF(const void *compressed_ttf_data,
    ...
    IMGUI_API ImFont *AddFontFromMemoryCompressedBase85TTF(const char *compressed_ttf_data_base85,
                                                           float size_pixels,
                                                           const ImFontConfig *font_cfg = NULL,
...
struct ImFont {
    ...
    ImVec2
        DisplayOffset;      // = (0.f,1.f)  // Offset font rendering by xx pixels
