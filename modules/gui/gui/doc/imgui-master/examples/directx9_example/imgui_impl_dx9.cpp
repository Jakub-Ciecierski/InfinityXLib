...
void ImGui_ImplDX9_RenderDrawLists(ImDrawData *draw_data) {
    ...
    if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount) {
        ...
        if (g_pd3dDevice
            ->CreateVertexBuffer(g_VertexBufferSize * sizeof(CUSTOMVERTEX),
    ...
    if (!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount) {
        ...
        if (g_pd3dDevice
            ->CreateIndexBuffer(g_IndexBufferSize * sizeof(ImDrawIdx),
                                D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY,
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                g_pd3dDevice
                    ->SetTexture(0, (LPDIRECT3DTEXTURE9) pcmd->TextureId);
                g_pd3dDevice->SetScissorRect(&r);
                g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,
                                                   vtx_offset,
                                                   0,
                                                   (UINT) cmd_list->VtxBuffer
                                                       .Size,
                                                   idx_offset,
                                                   pcmd->ElemCount / 3);
            }
...
void ImGui_ImplDX9_InvalidateDeviceObjects() {
    ...
    if (LPDIRECT3DTEXTURE9
        tex = (LPDIRECT3DTEXTURE9) ImGui::GetIO().Fonts->TexID) {
        tex->Release();
