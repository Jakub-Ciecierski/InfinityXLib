...
static uint32_t ImGui_ImplGlfwVulkan_MemoryType(VkMemoryPropertyFlags properties,
                                                uint32_t type_bits) {
...
void ImGui_ImplGlfwVulkan_RenderDrawLists(ImDrawData *draw_data) {
    ...
    if (!g_VertexBuffer[g_FrameIndex]
        ...
        vkGetBufferMemoryRequirements(g_Device,
                                      g_VertexBuffer[g_FrameIndex],
                                      &req);
        g_BufferMemoryAlignment =
    ...
    if (!g_IndexBuffer[g_FrameIndex]
        ...
        size_t index_buffer_size =
            ((index_size - 1) / g_BufferMemoryAlignment + 1)
                * g_BufferMemoryAlignment;
        VkBufferCreateInfo buffer_info = {};
        buffer_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                scissor.extent.height = (uint32_t)(
                    pcmd->ClipRect.w - pcmd->ClipRect.y + 1); // TODO: + 1??????
...
static void ImGui_ImplGlfwVulkan_SetClipboardText(void *user_data,
                                                  const char *text) {
    glfwSetClipboardString((GLFWwindow *) user_data, text);
}

void ImGui_ImplGlfwVulkan_MouseButtonCallback(GLFWwindow *,
                                              int button,
                                              int action,
                                              int /*mods*/) {
...
void ImGui_ImplGlfwVulkan_ScrollCallback(GLFWwindow *,
                                         double /*xoffset*/,
                                         double yoffset) {
    g_MouseWheel +=
...
void ImGui_ImplGlfwVulkan_KeyCallback(GLFWwindow *,
                                      int key,
                                      int,
                                      int action,
                                      int mods) {
...
bool ImGui_ImplGlfwVulkan_CreateDeviceObjects() {
    ...
    // Create The Shader Modules:
    {
        ...
        err = vkCreateShaderModule(g_Device,
                                   &vert_info,
                                   g_Allocator,
                                   &vert_module);
        ImGui_ImplGlfwVulkan_VkResult(err);
    ...
    if (!g_PipelineLayout) {
        ...
        err = vkCreatePipelineLayout(g_Device,
                                     &layout_info,
                                     g_Allocator,
                                     &g_PipelineLayout);
        ImGui_ImplGlfwVulkan_VkResult(err);
...
void ImGui_ImplGlfwVulkan_InvalidateDeviceObjects() {
    ...
    if (g_DescriptorSetLayout)
        vkDestroyDescriptorSetLayout(g_Device,
                                     g_DescriptorSetLayout,
                                     g_Allocator);
    if (g_PipelineLayout)
...
bool ImGui_ImplGlfwVulkan_Init(GLFWwindow *window,
    ...
    io.RenderDrawListsFn =
        ImGui_ImplGlfwVulkan_RenderDrawLists;       // Alternatively you can set this to NULL and call ImGui::GetDrawData() after ImGui::Render() to get the same ImDrawData pointer.
