...
void ImGui_ImplDX11_RenderDrawLists(ImDrawData *draw_data) {
    ...
    // Setup orthographic projection matrix into our constant buffer
    {
        ...
        if (ctx->Map(g_pVertexConstantBuffer,
                     0,
        ...
        VERTEX_CONSTANT_BUFFER
            *constant_buffer = (VERTEX_CONSTANT_BUFFER *) mapped_resource.pData;
        float L = 0.0f;
    ...
    ctx->IAGetIndexBuffer(&old.IndexBuffer,
    ...
    ctx->IASetIndexBuffer(old.IndexBuffer,
                          old.IndexBufferFormat,
                          old.IndexBufferOffset);
...
static void ImGui_ImplDX11_CreateFontsTexture() {
    ...
    // Upload texture to graphics system
    {
        ...
        g_pd3dDevice
            ->CreateShaderResourceView(pTexture, &srvDesc, &g_pFontTextureView);
        pTexture->Release();
    }
...
bool ImGui_ImplDX11_CreateDeviceObjects() {
    ...
    // Create the vertex shader
    {
        ...
        if (g_pVertexShaderBlob
            ...
            return false;
        if (g_pd3dDevice->CreateVertexShader((DWORD *) g_pVertexShaderBlob
                                                 ->GetBufferPointer(),
                                             g_pVertexShaderBlob
                                                 ->GetBufferSize(),
                                             NULL,
        ...
        D3D11_INPUT_ELEMENT_DESC local_layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
             (size_t)(&((ImDrawVert *) 0)->pos), D3D11_INPUT_PER_VERTEX_DATA,
             0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
             (size_t)(&((ImDrawVert *) 0)->uv), D3D11_INPUT_PER_VERTEX_DATA, 0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
             (size_t)(&((ImDrawVert *) 0)->col), D3D11_INPUT_PER_VERTEX_DATA,
             0},
        };
    ...
    // Create the pixel shader
    {
        ...
        if (g_pPixelShaderBlob
            == NULL)  // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
            return false;
        if (g_pd3dDevice->CreatePixelShader((DWORD *) g_pPixelShaderBlob
                                                ->GetBufferPointer(),
...
bool ImGui_ImplDX11_Init(void *hwnd,
                         ID3D11Device *device,
    ...
    io.RenderDrawListsFn =
        ImGui_ImplDX11_RenderDrawLists;  // Alternatively you can set this to NULL and call ImGui::GetDrawData() after ImGui::Render() to get the same ImDrawData pointer.
