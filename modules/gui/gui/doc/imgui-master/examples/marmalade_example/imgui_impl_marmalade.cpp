    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                pCurrentMaterial
                    ->SetDepthWriteMode(CIwMaterial::DEPTH_WRITE_NORMAL);
                pCurrentMaterial
                    ->SetAlphaTestMode(CIwMaterial::ALPHATEST_DISABLED);
...
int32 ImGui_Marmalade_PointerButtonEventCallback(void *SystemData,
                                                 void *pUserData) {
...
bool ImGui_Marmalade_CreateDeviceObjects() {
    ...
    image
        .SetBuffers();                                    // allocates and own buffers
...
bool ImGui_Marmalade_Init(bool install_callbacks) {
    ...
    io.KeyMap[ImGuiKey_Tab] =
        s3eKeyTab;                     // Keyboard mapping. ImGui will use those indices to peek into the io.KeyDown[] array.
    ...
    io.RenderDrawListsFn =
        ImGui_Marmalade_RenderDrawLists;      // Alternatively you can set this to NULL and call ImGui::GetDrawData() after ImGui::Render() to get the same ImDrawData pointer.
...
void ImGui_Marmalade_NewFrame() {
    ...
    io.MousePos = ImVec2((float) mouse_x / g_scale.x,
                         (float) mouse_y / g_scale
                             .y);   // Mouse position in screen coordinates (set to -1,-1 if no mouse / on another screen, etc.)
