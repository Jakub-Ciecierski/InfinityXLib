...
static uSynergyBool sSendReply(uSynergyContext *context) {
    ...
    // Send reply
    ret = context
        ->m_sendFunc(context->m_cookie, context->m_replyBuffer, reply_len);
...
static void sSendKeyboardCallback(uSynergyContext *context,
    ...
    // Send callback
    context
        ->m_keyboardCallback(context->m_cookie, key, modifiers, down, repeat);
}
...
static void sUpdateContext(uSynergyContext *context) {
    ...
    if (packlen > USYNERGY_RECEIVE_BUFFER_SIZE) {
        ...
        while (num_received != packlen) {
            ...
            int to_receive =
                buffer_left < USYNERGY_RECEIVE_BUFFER_SIZE ? buffer_left
...
void uSynergySendClipboard(uSynergyContext *context, const char *text) {
    ...
    sAddUInt32(context,
               1);                            /* Number of formats (only text for now) */
