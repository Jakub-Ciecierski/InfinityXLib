...
uSynergyBool ImGui_SendFunc(uSynergyCookie cookie,
...
uSynergyBool ImGui_RecvFunc(uSynergyCookie cookie,
                            uint8_t *buffer,
                            int maxLength,
                            int *outLength) {
...
void ImGui_MouseCallback(uSynergyCookie cookie,
                         uint16_t x,
                         uint16_t y,
                         int16_t wheelX,
                         int16_t wheelY,
...
void ImGui_KeyboardCallback(uSynergyCookie cookie,
                            uint16_t key,
                            uint16_t modifiers,
                            uSynergyBool down,
                            uSynergyBool repeat) {
...
//    printf("Synergy: keyboard callback: 0x%02X (%s)", scanCode, down?"true":"false");
    ...
    if ((down) && (key) && (scanCode < 256)
        && !(modifiers & USYNERGY_MODIFIER_CTRL)) {
        // If this key maps to a character input, apply it
        int charForKeycode = (modifiers & USYNERGY_MODIFIER_SHIFT)
                             ? g_keycodeCharShifted[scanCode]
...
void ImGui_JoystickCallback(uSynergyCookie cookie,
                            uint8_t joyNum,
                            uint16_t buttons,
                            int8_t leftStickX,
                            int8_t leftStickY,
...
void ImGui_ClipboardCallback(uSynergyCookie cookie,
...
static void ImGui_ImplIOS_RenderDrawLists(ImDrawData *draw_data) {
    ...
    glUniformMatrix4fv(g_AttribLocationProjMtx,
                       1,
                       GL_FALSE,
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        const int
            needed_vtx_size = cmd_list->VtxBuffer.Size * sizeof(ImDrawVert);
        if (g_VboSize < needed_vtx_size) {
            // Grow our buffer if needed
            g_VboSize = needed_vtx_size + 2000 * sizeof(ImDrawVert);
            glBufferData(GL_ARRAY_BUFFER,
                         (GLsizeiptr) g_VboSize,
                         NULL,
                         GL_STREAM_DRAW);
        }
