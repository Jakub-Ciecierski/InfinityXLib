...
RenderingEffectProcessor::CompileAllRenderingEffects() {
    ...
    for (auto &&dir : iterator) {
        if (IsValidProgramDirectory(dir.path().string())) {
            ifx::ProgramLoader
                program_loader(resource_context_->program_creator());
            try {
...
ShaderPaths RenderingEffectProcessor::GetShaderPaths(boost::filesystem::directory_entry dir) {
    ...
    for (auto &&file : dir_iterator) {
        ...
        switch (GetShaderType(string_path)) {
            case ShaderType::Vertex:shader_paths.vertex_path = string_path;
                break;
            case ShaderType::Fragment:shader_paths.fragment_path = string_path;
                break;
            case ShaderType::Geometry:shader_paths.geometry_path = string_path;
                break;
            case ShaderType::TessControl:shader_paths.tcs_path = string_path;
                break;
            case ShaderType::TessEvaluation:shader_paths.tes_path = string_path;
                break;
        }
    }
    return shader_paths;
